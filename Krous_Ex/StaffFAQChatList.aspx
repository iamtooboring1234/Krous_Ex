<%@ Page Title="" Language="C#" MasterPageFile="~/StaffMaster.Master" AutoEventWireup="true" CodeBehind="StaffFAQChatList.aspx.cs" Inherits="Krous_Ex.StaffFAQChatList" %>
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    ...
    <script>
        function EnterChatConfirmation() {
            if ($('#<%=hdCheckChatStatus.ClientID%>').val() == "Pending") {
                if (confirm('Are you sure to accept the chat ? \nOnce you accept the chat, you will in charge of this chat. ')) {
                    return true;
                } else {
                    return false;
                }
                                            ...
                                            <div class="row justify-content-center">
                                                <div class="col-md-8">
                                                    <asp:RadioButtonList ID="rblChatStatus" runat="server" CssClass="form-check-input mt-0" RepeatDirection="Horizontal" AutoPostBack="True" OnSelectedIndexChanged="rblChatStatus_SelectedIndexChanged">
                                                        <asp:ListItem Selected="True" Value="1">Pending (All)</asp:ListItem>
                                                        <asp:ListItem Value="2">In Progress</asp:ListItem>
                                                        <asp:ListItem Value="3">Ended</asp:ListItem>
                                                    </asp:RadioButtonList>
                                                </div>
                                            </div>
                                        </div>
                                            ...
                                            <asp:GridView ID="gvChat" runat="server" CssClass="table table-bordered tableChat" Width="100%" AutoGenerateColumns="False" DataKeyNames="ChatGUID" BorderStyle="none" CellPadding="10" CellSpacing="2">
                                                <Columns>
                                                    <asp:TemplateField ItemStyle-CssClass="text-center">
                                                        <ItemTemplate>
                                                            <asp:HyperLink runat="server" onclick="return EnterChatConfirmation();" NavigateUrl='<%# Eval("ChatGUID", "~/FAQChat.aspx?ChatGUID={0}") %>' Text="Enter Chat"/>
                                                        </ItemTemplate>
                                                    </asp:TemplateField>
                                                    <asp:TemplateField  ItemStyle-CssClass="text-center">
                                                        <ItemTemplate>
